...
class MYSMB(smb.SMB):
	def __init__(self, remote_host, use_ntlmv2=True, timeout=8):
		...
		self._smbConn = None
		super(self.__class__, self).__init__(remote_host, remote_host, timeout=timeout)
	...
	def neg_session(self, extended_security=True, negPacket=None):
		super(self.__class__, self).neg_session(self, extended_security=self.__use_ntlmv2, negPacket=negPacket)
	...
	def login(self, user, password, domain='', lmhash='', nthash='', ntlm_fallback=True, maxBufferSize=None):
		...
		super(self.__class__, self).login(user, password, domain, lmhash, nthash, ntlm_fallback)

	def login_standard(self, user, password, domain='', lmhash='', nthash='', maxBufferSize=None):
		_setup_login_packet_hook(maxBufferSize)
		super(self.__class__, self).login_standard(user, password, domain, lmhash, nthash)

	def login_extended(self, user, password, domain='', lmhash='', nthash='', use_ntlmv2=True, maxBufferSize=None):
		_setup_login_packet_hook(maxBufferSize)
		super(self.__class__, self).login_extended(user, password, domain, lmhash, nthash, use_ntlmv2)

	def connect_tree(self, path, password=None, service=smb.SERVICE_ANY, smb_packet=None):
		self._last_tid = super(self.__class__, self).tree_connect_andx(path, password, service, smb_packet)
	...
	def nt_create_andx(self, tid, filename, smb_packet=None, cmd=None, shareAccessMode=smb.FILE_SHARE_READ|smb.FILE_SHARE_WRITE, disposition=smb.FILE_OPEN, accessMask=0x2019f):
		self._last_fid = super(self.__class__, self).nt_create_andx(tid, filename, smb_packet, cmd, shareAccessMode, disposition, accessMask)
